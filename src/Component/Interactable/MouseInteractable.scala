...
trait MouseInteractable[C <: Component] {
  ...
  def onDrag(f: (MouseNotice, C) => Unit): Unit = interactor._onDrag = Some(f)
  ...
  def onDrag(f: => Unit): Unit = interactor._onExit = Some((_: MouseNotice, _: C) => f)
  ...
  def onDrag(f: C => Unit): Unit = interactor._onClick = Some((_: MouseNotice, c: C) => f(c))
